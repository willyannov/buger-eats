...
class SignupPage {
    ...
    fillForm(deliver) {
        ...
        cy.get('input[accept^="image" ]').attachFile('/images/' + deliver.cnh); //o attachFile vai buscar dentro da pasta /images/ o arquivo da CNH (UPLOAD)
    ...
    alertMessageShouldBe(expectateMessage) {
        // cy.get('.alert-error').should('have.text', expectateMessage)
        cy.contains('.alert-error', expectateMessage).should('be.visible');
    }
